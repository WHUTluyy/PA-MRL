...
from model import GNN,GNN_graphpred_codebook
...
def train(model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        num_part=torch.zeros(batch.id.shape[0],3)
        for i in range(batch.num_graphs):
            num_part[i,0]=torch.sum(batch[i].x[:,0]<119)
            num_part[i,1]=torch.sum(batch[i].x[:,0]==120)
            num_part[i,2]=torch.sum(batch[i].x[:,0]==119)
        pred = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, num_part)
...
def main():
    ...
    #set up model
    model = GNN_graphpred_codebook(args.num_layer, args.emb_dim, num_tasks, args=args,JK = args.JK, drop_ratio = args.dropout_ratio, gnn_type = args.gnn_type)
    ...
    if task_type == 'cls':
        ...
        for epoch in range(1, args.epochs+1):
            ...
            max_index = max(enumerate(val_auc_list), key=lambda x: x[1])[0]
            print("best_val and test auc:",val_auc_list[max_index]," ",test_auc_list[max_index])
